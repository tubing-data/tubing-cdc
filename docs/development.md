...
- **Docker Compose** — `docker-compose.yml` (repository root) defines:
  ...
  - **Quick-start CDC** — application built from the root `Dockerfile`; follows `cdc_test.orders`, processes its rows, and writes them to the `cdc-orders` Elasticsearch index. Run the complete smoke flow with `make demo`; see [quick-start.md](quick-start.md).
